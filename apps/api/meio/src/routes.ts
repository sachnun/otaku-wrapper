...
export const meioRoutes = new OpenAPIHono()
...
// Handlers
meioRoutes.openapi(listNovelsRoute, async (c) => {
	const { page } = c.req.valid('query')
	const result = await getNovelList(parseInt(page || '1'))
	return c.json(result)
})

meioRoutes.openapi(latestNovelsRoute, async (c) => {
	const novels = await getLatestUpdates()
	return c.json({ novels })
})

meioRoutes.openapi(searchNovelsRoute, async (c) => {
...
meioRoutes.openapi(novelDetailRoute, async (c) => {
...
meioRoutes.openapi(chapterListRoute, async (c) => {
	const { slug } = c.req.valid('param')
	const chapters = await getChapterList(slug)
	return c.json({ chapters })
})

meioRoutes.openapi(chapterContentRoute, async (c) => {
...
meioRoutes.openapi(listGenresRoute, async (c) => {
	const genres = await getGenres()
	return c.json({ genres })
})

meioRoutes.openapi(novelsByGenreRoute, async (c) => {
...
export const meioOpenApiConfig = {
	openapi: '3.0.0',
	info: {
